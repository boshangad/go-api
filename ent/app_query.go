...
// QueryAppOptions chains the current query on the "appOptions" edge.
func (aq *AppQuery) QueryAppOptions() *AppOptionQuery {
	query := &AppOptionQuery{config: aq.config}
...
func (aq *AppQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (aq *AppQuery) Clone() *AppQuery {
	...
	return &AppQuery{
		...
		withAppOptions: aq.withAppOptions.Clone(),
...
// WithAppOptions tells the query-builder to eager-load the nodes that are connected to
// the "appOptions" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AppQuery) WithAppOptions(opts ...func(*AppOptionQuery)) *AppQuery {
	query := &AppOptionQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withAppOptions = query
	return aq
}
...
func (aq *AppQuery) sqlAll(ctx context.Context) ([]*App, error) {
	...
	if query := aq.withAppOptions; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint64]*App)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.AppOptions = []*AppOption{}
		}
		query.Where(predicate.AppOption(func(s *sql.Selector) {
			s.Where(sql.InValues(app.AppOptionsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.AppOptions = append(node.Edges.AppOptions, n)
		}
	}
