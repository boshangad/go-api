...
func (auq *AppUserQuery) QueryApp() *AppQuery {
	query := &AppQuery{config: auq.config}
...
func (auq *AppUserQuery) QueryUser() *UserQuery {
	query := &UserQuery{config: auq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := auq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (auq *AppUserQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (auq *AppUserQuery) FirstIDX(ctx context.Context) uint64 {
...
func (auq *AppUserQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (auq *AppUserQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (auq *AppUserQuery) IDs(ctx context.Context) ([]uint64, error) {
	var ids []uint64
...
func (auq *AppUserQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := auq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (auq *AppUserQuery) WithUser(opts ...func(*UserQuery)) *AppUserQuery {
	query := &UserQuery{config: auq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(appuser.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.AppUser.Query().
//		Select(appuser.FieldCreateTime).
//		Scan(ctx, &v)
//
...
func (auq *AppUserQuery) sqlAll(ctx context.Context) ([]*AppUser, error) {
	...
	if query := auq.withApp; query != nil {
		...
		nodeids := make(map[uint64][]*AppUser)
		for i := range nodes {
			fk := nodes[i].AppID
	...
	if query := auq.withUser; query != nil {
		ids := make([]uint64, 0, len(nodes))
		nodeids := make(map[uint64][]*AppUser)
		for i := range nodes {
			fk := nodes[i].UserID
