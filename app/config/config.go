...
type Config struct {
	...
	// 磁盘功能
	Disk map[string]map[string]interface{} `json:"disk,omitempty" yaml:"disk,omitempty"`
	// 缓存功能模块
	Cache map[string]map[string]interface{} `json:"cache,omitempty" yaml:"cache"`
	// Redis 服务器
	Redis map[string]map[string]interface{} `json:"redis,omitempty" yaml:"redis"`
	// 短信网关
	Sms map[string]map[string]interface{} `json:"sms,omitempty" yaml:"sms"`
	// 邮件网关
	Email map[string]map[string]interface{} `json:"email,omitempty" yaml:"email"`
	// 验证码配置
