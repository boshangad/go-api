...
// Send 发送短信
func (that *Ucloud) Send(to interfaces.PhoneNumber, message interfaces.Message, config interfaces.Config) (interfaces.Result, error) {
	var (
...
// 生产请求参数
func (that Ucloud) buildParams(to interfaces.PhoneNumber, message interfaces.Message, config interfaces.Config) (params url.Values) {
	var (
		data = message.GetData(&that)
	)
	params.Set("Action", that.requestAction)
	params.Set("SigContent", config.GetString("sigContent"))
	...
	params.Set("Signture", that.signtureParams(params, config.GetString("privateKey")))
	return params
}
...
func (config Ucloud) signtureParams(data url.Values, privateKey string) string {
	...
	sort.Strings(keys)
	for _, k = range keys {
		signtureStr += fmt.Sprintf("%s%v", k, data[k])
	}
...
// 实例化短信
func NewUcloudGateway(gateway map[string]interface{}) *Ucloud {
	return &Ucloud{
