package core

import (
	"flag"
	"fmt"
	"os"
	"path/filepath"

	"github.com/boshangad/go-api/config"
	viper2 "github.com/boshangad/go-api/core/viper"
	"github.com/boshangad/go-api/global"
	"github.com/fsnotify/fsnotify"
	jww "github.com/spf13/jwalterweatherman"
	"github.com/spf13/viper"
)

// Viper 初始化Viper配置加载
func Viper(path ...string) *viper.Viper {
	var c string
	if len(path) == 0 {
		flag.StringVar(&c, "c", "", "choose config file.")
		flag.Parse()
		// 优先级: 命令行 > 环境变量 > 默认值
		if c == "" {
			if configEnv := os.Getenv(viper2.ConfigEnv); configEnv == "" {
				c = viper2.ConfigFile
				fmt.Printf("您正在使用config的默认值,config的路径为%v\n", viper2.ConfigFile)
			} else {
				c = configEnv
				fmt.Printf("您正在使用G_CONFIG环境变量,config的路径为%v\n", c)
			}
		} else {
			fmt.Printf("您正在使用命令行的-c参数传递的值,config的路径为%v\n", c)
		}
	} else {
		c = path[0]
		fmt.Printf("您正在使用func Viper()传递的值,config的路径为%v\n", c)
	}
	jww.SetStdoutThreshold(jww.LevelTrace)
	v := viper.New()
	v.AddConfigPath(filepath.Dir(os.Args[0]))
	//v.SetConfigFile(config)
	v.SetConfigType("yaml")
	err := v.ReadInConfig()
	if err != nil {
		panic(fmt.Errorf("Fatal error config file: %s \n", err))
	}
	// 监听配置文件修改
	v.WatchConfig()
	// 配置文件修改
	v.OnConfigChange(func(e fsnotify.Event) {
		fmt.Println("config file changed:", e.Name)
		if err := v.Unmarshal(&global.G_CONFIG); err != nil {
			fmt.Println(err)
		}
	})
	if err := v.Unmarshal(&global.G_CONFIG); err != nil {
		fmt.Println(err)
	}
	initSetDefaultConfig(&global.G_CONFIG)
	global.G_CONFIG.App.Root, _ = filepath.Abs("..")
	return v
}

// 初始设置默认的配置
func initSetDefaultConfig(conf *config.Server) {
	// 应用初始化
	if conf.App.Root == "" {
	}
	// 验证码默认初始化
	if conf.Captcha == nil {
		conf.Captcha = map[string]config.Captcha{
			"image": {
				Type:          "image", // 类型
				Length:        0,       // 字符长度
				MinLength:     4,       // 最小字符长度
				MaxLength:     8,       // 最大字符长度
				NoiseCount:    0,
				MinNoiseCount: 0,
				MaxNoiseCount: 8,
			},
			"audio": {
				Type:     "audio",
				Length:   4,
				Language: "zh",
			},
		}
	}
	for key, captcha := range conf.Captcha {
		if captcha.Language == "" {
			captcha.Language = "zh"
		}
		conf.Captcha[key] = captcha
	}
}
